...
<div id="content" class="px-5 pt-5">
		...
		<p class="card-text m-0"><small class="text-muted"><i class="icofont-filter"></i> Filtro</small></p>
			<div class="form-row">
				<div class="col">
					...
					<select class="selectpicker" id="sltPAnios" data-live-search="true" data-width="auto">
						<?php include 'php/returnAniosDocenteCiclaje.php'; ?>
					</select>
				</div>
				<div class="col">
				<label for=""><small>Mes:</small></label>
					<select class="selectpicker" id="sltPMeses" data-live-search="true" data-width="auto">
				...
				<div class="col d-flex align-items-end">
					<button class="btn btn-outline-primary"><i class="icofont-search-1"></i> Buscar</button>
				</div>
			</div>
		</div>
	</div>
	...
	where Emp_Codigo = '{$_COOKIE['ckidUsuario']}' and year(ma.Mes_Inicio)= {$_GET['year']} and month(ma.Mes_Inicio)= {$_GET['month']}
	order by ma.Mes_Inicio desc; "; //and Sec_Detalle ='Habilitado' echo $sqlCursos;
	$resultadoCursos=$cadena->query($sqlCursos); $i=1; ?>
	<p>Su lista de cursos asignados:</p>
	<div class="container">
...
$('#sltPAnios').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
	if( $('#sltPAnios').selectpicker('val')!= null ){
		location.href = "docente.php?year="+$('#sltPAnios').selectpicker('val');
	}
});
$('#sltPMeses').on('changed.bs.select', function (e, clickedIndex, isSelected, previousValue) {
	if( $('#sltPMeses').selectpicker('val')!= null ){
		location.href = "docente.php?year="+$('#sltPAnios').selectpicker('val')+'&month='+$('#sltPMeses').selectpicker('val');
	}
});
